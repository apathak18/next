...
class NextSite extends ConfigEntityBase implements NextSiteInterface {
  ...
  public function getPreviewUrlForEntity(EntityInterface $entity): Url {
    ...
    // Same for authenticated users with no additional roles,
    // since we assume no scope.
    ...
    if ($next_entity_type_manager->isEntityRevisionable($entity)) {
      ...
      // In Drupal terms, a "working copy" is the latest revision. It may or
      // may not be a "default" revision. This revision is the working copy
      // because it is the revision to which new work will be applied.
      // In other words, it denotes the most recent revision
      // which might be considered a work-in-progress.
      // @see \Drupal\jsonapi\Revisions\VersionByRel
      if ($entity->isLatestRevision()) {
        $rel = 'working-copy';
      }

      // In Drupal terms, the "latest version" is latest "default" revision.
      ...
      // becauseit is the last revision where work was considered finished.
    ...
    // This allows next.js app to determine the preview strategy.
